...
using array_for_arma_t = nb::ndarray<
    ...
    nb::numpy, nb::ndim<ndim_v<ArmaType>>, nb::f_contig, nb::device::cpu>;
...
template <> struct arma_for_array<arr1d> {
  using type = arma::dvec;
};

template <> struct arma_for_array<arr2d> {
  using type = arma::dmat;
};

template <> struct arma_for_array<arr3d> {
...
template <typename ArmaType> auto ToPy(ArmaType &&arma_obj) {
  using Arma = std::decay_t<ArmaType>;
  using Numpy =
      std::conditional_t<std::is_const_v<std::remove_reference_t<ArmaType &&>>,
                         array_for_arma_t<const Arma>, array_for_arma_t<Arma>>;

  if constexpr (std::is_rvalue_reference_v<ArmaType &&>) {
    auto *ptr = new Arma(std::forward<ArmaType>(arma_obj));
    nb::capsule owner(ptr,
                      [](void *p) noexcept { delete static_cast<Arma *>(p); });
    ...
    if constexpr (ndim_v<Arma> == 3) {
      return Numpy(ptr->memptr(), {ptr->n_rows, ptr->n_cols, ptr->n_slices},
                   owner);
    }
...
auto ToArma(Numpy arr) {
  ...
  if constexpr (arma::is_Col<ArmaType>::value) {
    ...
    return ArmaType(arr.data(), arr.shape(0), arr.shape(1), arr.shape(2), false,
                    true);
  }
}
