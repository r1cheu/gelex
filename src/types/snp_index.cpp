...
SnpIndex::SnpIndex(size_t initial_capacity)
{
...
auto SnpIndex::emplace(SnpMeta meta) -> void
{
...
auto SnpIndex::find_index(std::string_view snp_id) const
...
auto SnpIndex::find(std::string_view snp_id) -> SnpMeta*
{
...
auto SnpIndex::find(std::string_view snp_id) const -> const SnpMeta*
{
    auto idx = find_index(snp_id);
    if (idx)
    {
...
auto SnpIndex::shrink_to_fit() -> void
{
    snp_meta_.shrink_to_fit();
}

auto SnpIndex::clear() -> void
{
    snp_meta_.clear();
