...
auto SingleSharedGaussianStep::step() -> void
{
    ...
    variance_ = variance_sampler_({coeffs.size(), sum_squares}, rng_);
    ...
    : design_(design),
      ...
      proportion_sampler_(
          [&prior] -> std::optional<stats::DirichletSampler<double>>
          {
              const auto& concrete
                  = std::get<bayes::SingleSharedSpikeSlabGaussianPrior>(prior);
              if (!concrete.proportion().prior())
              {
                  return std::nullopt;
              }
              return stats::DirichletSampler<double>{
                  concrete.proportion().prior()->concentration()};
          }()),
      state_(block.state()),
      residual_(residual),
      normal_(variance_),
    ...
    for (Eigen::Index i = 0; i < X.cols(); ++i)
    {
        if (!design_.is_monomorphic(i))
        {
            ...
            const double rhs
                = column.dot(residual_.y_adj) + (XtX_diag(i) * old_i);
            const auto post
                = normal_.set_prior_var(variance_).posterior_with_logL(
                    stats::NormalSampler<double>::Kernel{
                        .quadratic = XtX_diag(i),
                        .linear = rhs,
                        .scale = residual_.variance,
...
SinglePerMarkerSpikeSlabStep::SinglePerMarkerSpikeSlabStep(
    ...
    : design_(design),
      ...
      proportion_sampler_(
          ...
          {
              const auto& concrete
                  = std::get<bayes::SinglePerMarkerSpikeSlabGaussianPrior>(
                      prior);
              if (!concrete.proportion().prior())
              {
                  return std::nullopt;
