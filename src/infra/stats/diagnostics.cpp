...
std::pair<VectorXd, VectorXd> compute_chain_variance_stats(const Chains& x)
{
    ...
    for (Index c = 0; c < n_chains; ++c)
    {
        ...
        chain_vars.col(c)
            = detail::matvar<1>(x[c], detail::VarNormType::Sample);
    }
    ...
    if (n_chains > 1)
    {
        MatrixXd var_between
            = detail::matvar<1>(chain_means, detail::VarNormType::Sample);
...
Chains autocovariance(const Chains& x, bool bias)
{
    ...
    {
        x_var.col(i) = detail::matvar<1>(x[i], detail::VarNormType::Population);
    }
