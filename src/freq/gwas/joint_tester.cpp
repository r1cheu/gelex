...
#pragma omp parallel for schedule(static)
    ...
    // per-effect PVE
    add_.pve = matvar(Z_a_ * add_.beta.asDiagonal(), VarNormType::Population)
                   .transpose()
                   .array()
               / reml.Vp;
    dom_.pve = matvar(Z_d_ * dom_.beta.asDiagonal(), VarNormType::Population)
                   .transpose()
                   .array()
               / reml.Vp;
    ...
    total_pve_
        = matvar(pred, VarNormType::Population).transpose().array() / reml.Vp;
