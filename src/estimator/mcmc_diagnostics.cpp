...
arma::dmat compute_chain_variance_stats(const arma::dcube& x) {
    const int n_chains = x.n_slices;
    const int n_draws = x.n_cols;
    ...
    arma::dmat var_estimator = var_within * (n_draws - 1.0) / n_draws;
...
arma::dmat split_gelman_rubin(const arma::dcube& samples) {
    const int N_half = samples.n_cols / 2;
    ...
    for(int c = 0; c < samples.n_slices; ++c) {
        ...
        new_input.slice(c).cols(N_half, 2*N_half - 1) = 
            samples.slice(c).cols(samples.n_cols - N_half, samples.n_cols - 1);
    }
...
arma::dmat autocorrelation(const arma::dmat& chain) {
    const int N = chain.n_cols;
    const int M = fft_next_fast_len(N);
    const int M2 = 2 * M;
    ...
    for(int r = 0; r < chain.n_rows; ++r) {
        arma::vec centered = arma::vectorise(chain.row(r) - 
                                  arma::mean(chain.row(r)));
