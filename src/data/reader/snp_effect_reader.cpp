...
SnpEffectReader::SnpEffectReader(const std::filesystem::path& snp_effect_path)
{
    try
    {
...
void SnpEffectReader::load(const std::filesystem::path& snp_effect_path)
{
...
void SnpEffectReader::parse_header(
...
void SnpEffectReader::parse_line(
    std::string_view line,
    int line_number,
    const ColumnIndices& indices)
{
    ...
    try
    {
        ...
        {
            snp_effects_.emplace(std::move(meta), add_val, a1_freq);
        }
        else
        {
            snp_effects_.emplace(std::move(meta), add_val, dom_val, a1_freq);
        }
    }
    catch (const GelexException& e)
    {
...
ColumnIndices SnpEffectReader::assign_column_indices(
