...
[[nodiscard]] auto select_kernel(
    Kernel scalar_kernel,
    Kernel avx2_kernel,
    Kernel avx512_kernel) noexcept -> Kernel
{
    if (supports_avx512())
    {
...
auto select_dot_kernel() noexcept -> DotKernel
{
    return select_kernel<DotKernel>(dot_scalar, dot_avx2, dot_avx512);
}

auto select_axpy_kernel() noexcept -> AxpyKernel
{
    return select_kernel<AxpyKernel>(axpy_scalar, axpy_avx2, axpy_avx512);
}

auto select_multi_target_axpy_kernel() noexcept -> MultiTargetAxpyKernel
{
    return select_kernel<MultiTargetAxpyKernel>(
    ...
    std::span<const double> residual) noexcept -> double
{
    ...
    static const detail::DotKernel kernel = detail::select_dot_kernel();
    return kernel(
...
{
    assert(genotype_column.size() == residual.size());
    static const detail::AxpyKernel kernel = detail::select_axpy_kernel();
    kernel(
    ...
    std::span<const AxpyTarget> targets) noexcept -> void
{
    ...
    static const detail::MultiTargetAxpyKernel kernel
        = detail::select_multi_target_axpy_kernel();
    kernel(genotype_column.data(), lut.data(), targets, genotype_column.size());
}
