...
auto MCMC<TraitSampler>::run(
    ...
    const FitObserver& observer) -> MCMCResult
{
    MCMCSamples samples(model, priors, sample_prefix, params_.n_records);
...
template <typename TraitSampler>
void MCMC<TraitSampler>::validate_checkpoint(
    const BayesState& state,
    const BayesModel& model)
{
    auto check
...
template <typename TraitSampler>
auto MCMC<TraitSampler>::finalize(
    MCMCSamples samples,
    const BayesModel& model,
    const FitObserver& observer) -> MCMCResult
{
