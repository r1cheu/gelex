...
/*! \mainpage The documentation of the single C++ file *vcfpp.h* for
 * manipulating VCF/BCF
 *
 * \section intro_sec Introduction
 *
 * This project https://github.com/Zilong-Li/vcfpp introduces a single C++ file
 * as interface to the basic htslib, which can be easily included in a C++
 ...
 *
 * - <EM> include "vcfpp.h" </EM> to your program and compile it by <EM> g++
 * my.cpp -std=c++11 -Wall -I. -lhts
 * - </EM>
 * - make sure you have https://github.com/samtools/htslib installed on your
 * system and the it is in your environment.
 *
 *
 * \copyright Copyright (C) 2022 Zilong Li . This project is governed by the
 * LICENSE file in https://github.com/Zilong-Li/vcfpp.
 *
 *
 */
...
template <typename T>
using isValidInfo = typename std::enable_if<
    std::is_same<T, std::string>::value
        || std::is_same<T, std::vector<int>>::value
        || std::is_same<T, std::vector<float>>::value,
    bool>::type;
...
template <typename T>
using isScalar = typename std::enable_if<
...
/**
 * @class BcfHeader
 * @brief Object represents header of the VCF, offering methods to access and
 *modify the tags
...
class BcfHeader
{
   ...
   public:
    ...
    inline std::string asString() const
    {
        ...
        if (bcf_hdr_format(hdr, 0, &s)
            == 0)  // append header string to s.s! append!
        {
    ...
    inline int getFormatType(std::string tag) const
    {
        ...
        if (bcf_hdr_id2type(hdr, BCF_HL_FMT, tag_id) == (BCF_HT_INT & 0xff))
        {
            ...
            bcf_hdr_id2type(hdr, BCF_HL_FMT, tag_id) == (BCF_HT_REAL & 0xff))
        {
            return 2;
        }
        else if (
            bcf_hdr_id2type(hdr, BCF_HL_FMT, tag_id) == (BCF_HT_STR & 0xff))
        {
            return 3;
        }
        else
                ...
                45))
        {
            if (i > 0
                && !strncmp(
                    ...
                    38))
                throw std::runtime_error(
                    "Error: missing FORMAT fields, cowardly refusing to add "
                    "samples\n");
            throw std::runtime_error(
    ...
    inline int nSamples() const { return bcf_hdr_nsamples(hdr); }
};

/**
 * @class BcfRecord
 * @brief Object represents a variant record in the VCF, offering methods to
 *access and modify fields.
 * @note  BcfRecord has to be associated with a BcfHeader object and needs to be
 *filled in by calling BcfReader.getNextVariant function.
 **/
class BcfRecord
{
    friend class BcfReader;
   ...
   private:
    BcfHeader* header;
    std::shared_ptr<bcf1_t> line = std::shared_ptr<bcf1_t>(
        bcf_init(),
        details::bcf_line_close());  // variant
    ...
    bool noneMissing
        = true;  // whenever parsing a tag have to reset this variable
   ...
   public:
    ...
    BcfRecord(BcfHeader& h) { initHeader(h); }
    ...
    /// initilize the header associated with BcfRecord object by pointing to
    /// another BcfHeader object
    void initHeader(BcfHeader& h)
    {
        header = &h;
        if (!header->hdr)
            throw std::runtime_error("please initial header first\n");
        nsamples = header->nSamples();
        typeOfGT.resize(nsamples);
        gtPhase.resize(nsamples, 0);
    }

    /// reset the header associated with BcfRecord object by pointing to another
    /// BcfHeader object
    void resetHeader(BcfHeader& h) { header = &h; }
    ...
    /**
     * @brief fill in the input vector with genotypes of 0 and 1. only works for
     * ploidy<=2. Genotypes with missing allele is coded as heterozygous
     * @param v valid input includes vector<bool> and vector<char> type
     * @return bool
     * @note  use isNoneMissing() to check if all genotypes are with no
     * missingness. Alternatively, one can use vector<int> as the input type as
     * noted in the other overloading function getGenotypes().
     * */
    template <typename T>
    isValidGT<T> getGenotypes(T& v)
    {
        ...
        {
#if defined(VERBOSE)
            std::cerr << "GT not present for current site. did you initilize "
                         "the variant object?\n";
#endif
            return false;
        }
        // if nploidy is not set manually. find the max nploidy using the first
        // variant (eg. 2) resize v as max(nploidy)
        // * nsamples (ret) NOTE: if ret == nsamples and only one sample then
        // haploid
        if (ret != nploidy * nsamples && nploidy > 0)
        {
            // rare case if nploidy is set manually. eg. only one sample. the
            // first variant is '1' but the second is '1/0'. ret = 1 but nploidy
            // should be 2
        ...
        for (i = 0; i < nsamples; i++)
        {
            // check and fill in typeOfGT; only supports SNPs now. check
            // vcfstats.c for inspiration
            typeOfGT[i] = bcf_gt_type(fmt, i, NULL, NULL);
            if (typeOfGT[i] == GT_UNKN)
            {
                noneMissing = false;  // set missing as het, user should check
                                      // if isNoneMissing();
                isGenoMissing[i] = 1;
                v[i * nploidy + 0] = 1;
                for (j = 1; j < nploidy_cur; j++)
                    v[i * nploidy + j] = 0;
                continue;
            }
            ...
            {
                // TODO: right now only parse 0 and 1, ie max(nploidy)=2; other
                // values 2,3... will be set to 1
                v[i * nploidy + j]
                    = bcf_gt_allele(gts[j + i * nploidy_cur]) != 0;
            }
            if (nploidy == 2)
            {
    ...
    /**
     * @brief fill in the input vector with genotype values, 0, 1 or -9
     * (missing).
     * @param v valid input is vector<int> type
     * @return bool
     * @note this function provides full capability to handle all kinds of
     * genotypes in multi-ploidy data costing more spae than the other function.
     * missing allele is set as -9.
     * */
    ...
    isFormatVector<T> getFORMAT(std::string tag, T& v)
    {
        ...
        if (!fmt)
        {
            throw std::invalid_argument(
                "no FORMAT=" + tag + " in the VCF header.\n");
        }
        nvalues = fmt->n;
        ndst = 0;
        S* dst = NULL;
        ...
        if (tagid == 1)
        {
            ret = bcf_get_format_int32(
                header->hdr, line.get(), tag.c_str(), &dst, &ndst);
        }
        else if (tagid == 2)
        {
            ret = bcf_get_format_float(
                header->hdr, line.get(), tag.c_str(), &dst, &ndst);
        }
        else if (tagid == 3)
        {
            ret = bcf_get_format_char(
                header->hdr, line.get(), tag.c_str(), &dst, &ndst);
        }
        else
        {
    ...
    bool getFORMAT(std::string tag, std::vector<std::string>& v)
    {
        ...
        if (ret > 0)
        {
            ...
            {
                // Ugly: GT field is considered to be a string by the VCF header
                // but BCF represents it as INT.
    ...
    isInfoVector<T> getINFO(std::string tag, T& v)
    {
        ...
        {
            ret = bcf_get_info_int32(
                header->hdr, line.get(), tag.c_str(), &dst, &ndst);
        }
        else if (info->type == BCF_BT_FLOAT)
        {
            ret = bcf_get_info_float(
                header->hdr, line.get(), tag.c_str(), &dst, &ndst);
        }
        if (ret >= 0)
        {
            v = std::vector<S>(dst, dst + ret);  // user have to check if there
                                                 // is missing in the return v;
            free(dst);
            return true;
        }
        else
    ...
    isString<T> getINFO(std::string tag, T& v)
    {
        ...
        {
            v = std::string(
                reinterpret_cast<char*>(info->vptr), info->vptr_len);
            return true;
        }
        else
        {
    ...
    isScalar<T> setINFO(std::string tag, const T& v)
    {
        ...
        if (bcf_hdr_id2type(header->hdr, BCF_HL_INFO, tag_id)
            == (BCF_HT_INT & 0xff))
        {
            ret = bcf_update_info_int32(
                header->hdr, line.get(), tag.c_str(), &v, 1);
        }
        else if (
            bcf_hdr_id2type(header->hdr, BCF_HL_INFO, tag_id)
            == (BCF_HT_REAL & 0xff))
        {
            float v2 = static_cast<float>(v);
            ret = bcf_update_info_float(
                header->hdr, line.get(), tag.c_str(), &v2, 1);
        }
        else
        {
            ret = -1;
    ...
    isValidInfo<T> setINFO(std::string tag, const T& v)
    {
        ...
        {
            ret = bcf_update_info_int32(
                header->hdr, line.get(), tag.c_str(), v.data(), v.size());
        }
        else if (
            bcf_hdr_id2type(header->hdr, BCF_HL_INFO, tag_id)
            == (BCF_HT_REAL & 0xff))
            ...
            == (BCF_HT_STR & 0xff))
        {
            ret = bcf_update_info_string(
                header->hdr, line.get(), tag.c_str(), v.data());
        }
        else
        {
            ret = -1;
    ...
    void removeINFO(std::string tag)
    {
        ...
        {
            ret = bcf_update_info_int32(
                header->hdr, line.get(), tag.c_str(), NULL, 0);
        }
        else if (
            bcf_hdr_id2type(header->hdr, BCF_HL_INFO, tag_id)
            == (BCF_HT_REAL & 0xff))
        ...
        {
            ret = bcf_update_info_string(
                header->hdr, line.get(), tag.c_str(), NULL);
        }
        else
        {
            ret = -1;
    ...
    void setPhasing(const std::vector<char>& v)
    {
        if ((int)v.size() != nsamples)
            throw std::runtime_error(
                "the size of input vector is not matching the size of "
                "genotypes");
        gtPhase = v;
    }
    ...
    void removeFORMAT(std::string tag)
    {
        ret = -1;
        int tag_id = bcf_hdr_id2int(header->hdr, BCF_DT_ID, tag.c_str());
        if (bcf_hdr_id2type(header->hdr, BCF_HL_FMT, tag_id)
            == (BCF_HT_INT & 0xff))
        {
            ret = bcf_update_format_int32(
                header->hdr, line.get(), tag.c_str(), NULL, 0);
        }
        else if (
            bcf_hdr_id2type(header->hdr, BCF_HL_FMT, tag_id)
            == (BCF_HT_STR & 0xff))
        {
            ret = bcf_update_format_char(
                header->hdr, line.get(), tag.c_str(), NULL, 0);
        }
        else if (
            bcf_hdr_id2type(header->hdr, BCF_HL_FMT, tag_id)
     ...
     * @param tag valid tag name in FORMAT column declared in the VCF header
     * @param v valid input include vector<int>, vector<float>, vector<char>,
     * std::string
     * @return bool
     * */
    template <typename T>
    ...
    /**
     * @brief set tag for a single sample in FORMAT using given singular value.
     * this works only when there is one sample in the vcf
     * @param tag valid tag name in FORMAT column declared in the VCF header
    ...
    void addLineFromString(const std::string& vcfline)
    {
        ...
        if (line->errcode == BCF_ERR_CTG_UNDEF)
        {
            ...
            if (ret < 0)
                throw std::runtime_error(
                    "error adding contig " + contig + " to header.\n");
            ret = bcf_hdr_sync(header->hdr);
        }
    }

    /** @brief if all samples have non missing values for the tag in FORMAT */
    inline bool isNoneMissing() const { return noneMissing; }

    /** @brief return boolean value indicates if current variant is Structual
     * Variant or not */
    ...
    /** @brief return boolean value indicates if current variant is exclusively
     * INDEL */
    ...
    /** @brief return boolean value indicates if current variant is multiallelic
     * sites */
    ...
    /** @brief return boolean value indicates if current variant is exclusively
     * biallelic SNP. Note ALT=* are skipped */
    ...
    /** @brief return boolean value indicates if current variant has SNP type
     * defined in vcf.h (htslib>=1.16)
     */
    inline bool hasSNP() const
    {
        int type
            = bcf_has_variant_types(line.get(), VCF_SNP, bcf_match_overlap);
        if (type < 0)
            throw std::runtime_error("something wrong with variant type\n");
        if (type == 0)
            return false;
        return true;
    }

    /// return boolean value indicates if current variant has INDEL type defined
    /// in vcf.h (htslib>=1.16)
    inline bool hasINDEL() const
    {
        int type
            = bcf_has_variant_types(line.get(), VCF_INDEL, bcf_match_overlap);
        if (type < 0)
            throw std::runtime_error("something wrong with variant type\n");
        if (type == 0)
            return false;
    ...
    /// return boolean value indicates if current variant has INS type defined
    /// in vcf.h (htslib>=1.16)
    inline bool hasINS() const
    {
        int type
            = bcf_has_variant_types(line.get(), VCF_INS, bcf_match_overlap);
        if (type < 0)
            throw std::runtime_error("something wrong with variant type\n");
        if (type == 0)
            return false;
    ...
    /// return boolean value indicates if current variant has DEL type defined
    /// in vcf.h (htslib>=1.16)
    inline bool hasDEL() const
    {
        int type
            = bcf_has_variant_types(line.get(), VCF_DEL, bcf_match_overlap);
        if (type < 0)
            throw std::runtime_error("something wrong with variant type\n");
        if (type == 0)
            return false;
    ...
    /// return boolean value indicates if current variant has MNP type defined
    /// in vcf.h (htslib>=1.16)
    inline bool hasMNP() const
    {
        int type
            = bcf_has_variant_types(line.get(), VCF_MNP, bcf_match_overlap);
        if (type < 0)
            throw std::runtime_error("something wrong with variant type\n");
        if (type == 0)
            return false;
    ...
    /// return boolean value indicates if current variant has BND type defined
    /// in vcf.h (htslib>=1.16)
    inline bool hasBND() const
    {
        int type
            = bcf_has_variant_types(line.get(), VCF_BND, bcf_match_overlap);
        if (type < 0)
            throw std::runtime_error("something wrong with variant type\n");
        if (type == 0)
            return false;
    ...
    /// return boolean value indicates if current variant has OTHER type defined
    /// in vcf.h (htslib>=1.16)
    inline bool hasOTHER() const
    {
        int type
            = bcf_has_variant_types(line.get(), VCF_OTHER, bcf_match_overlap);
        if (type < 0)
            throw std::runtime_error("something wrong with variant type\n");
        if (type == 0)
            return false;
    ...
    /// return boolean value indicates if current variant has OVERLAP type
    /// defined in vcf.h (htslib>=1.16)
    inline bool hasOVERLAP() const
    {
        int type
            = bcf_has_variant_types(line.get(), VCF_OVERLAP, bcf_match_overlap);
        if (type < 0)
            throw std::runtime_error("something wrong with variant type\n");
        if (type == 0)
            return false;
    ...
    inline std::string ID() const { return std::string(line->d.id); }

    /** @brief return 1-base position */
    inline int64_t POS() const { return line->pos + 1; }
    ...
    inline void setPOS(int64_t p) { line->pos = p - 1; }
    ...
    inline void setQUAL(float q) { line->qual = q; }

    /** @brief modify the QUAL value */
    inline void setQUAL(char q) { bcf_float_set_missing(line->qual); }
    ...
    inline int64_t Start() const { return line->pos; }

    /** @brief return 0-base end of the variant (can be any type) */
    inline int64_t End() const { return line->pos + line->rlen; }

    /** @brief return raw REF alleles as string */
    inline std::string REF() const { return std::string(line->d.allele[0]); }

    /** @brief swap REF and ALT for biallelic SNP */
    inline void swap_REF_ALT()
    {
        if (!isMultiAllelicSNP())
    ...
    inline std::string FILTER()
    {
        ...
        {
            return std::string(
                bcf_hdr_int2id(header->hdr, BCF_DT_ID, line->d.flt[0]));
        }
        else
        {
            std::string s;
            ...
            {
                s += std::string(
                         bcf_hdr_int2id(header->hdr, BCF_DT_ID, line->d.flt[i]))
                     + ",";
            }
            s.pop_back();
            return s;
    ...
    /** @brief return raw INFO column as string. recommend to use getINFO for
     * specific tag. */
        ...
        if (line->n_info)
        {
            ...
            for (int i = 0; i < line->n_info; ++i)
            {
                ...
                if (z->key < 0 || z->key >= max_dt_id
                    || header->hdr->id[BCF_DT_ID][z->key].key == NULL)
    ...
    /** @brief return boolean value indicates if genotypes of all samples are
     * phased */
    inline bool allPhased() const { return isAllPhased; }

    /** @brief return the number of ploidy of current variant */
    inline int ploidy() const { return nploidy; }

    /** @brief in a rare case, one may want to set the number of ploidy manually
     */
    inline void setPloidy(int v) { nploidy = v; }

    /**
     * @brief vector of nsamples length. keep track of the type of genotype (one
     * of GT_HOM_RR, GT_HET_RA, GT_HOM_AA, GT_HET_AA, GT_HAPL_R, GT_HAPL_A or
     * GT_UNKN).
     * @note GT_HOM_RR 0 \n
     *       GT_HOM_AA 1 \n
     *       GT_HET_RA 2 \n
    ...
    /** @brief vector of nsamples length. keep track of the phasing status of
     * each sample */
...
class BcfReader
{
   ...
   public:
     ...
     * "-"
     */
    BcfReader(const std::string& file) : fname(file) { open(file); }
     ...
     *  @param region samtools-like region "chr:start-end", skip if empty
     *  @param samples  LIST samples to include or exclude as a comma-separated
     * string. \n LIST : select samples in list \n ^LIST : exclude samples from
     * list \n
     *                  "-" : include all samples \n
     *                  "" : exclude all samples
     */
    ...
    void open(const std::string& file)
    {
        ...
        fname = file;
        fp = std::shared_ptr<htsFile>(
            hts_open(fname.c_str(), "r"), details::hts_file_close());
        if (!fp)
            throw std::invalid_argument("I/O error: input file is invalid");
     ...
     * @note If you want to continue work on that region, remember to reset the
     * region by setRegion()! \n Also, check the status of the region first to
     ...
     * @param samples the string is bcftools-like format, which is comma
     * separated list of samples to include (or exclude with "^" prefix).
     * */
    ...
    /**
     * @brief explicitly stream to specific region. throw invalid_argument error
     * if index file not found. throw runtime_error if the region was not a
     * valid bcftools-like format or was not presenting in the VCF.
     * @param region the string for region is samtools-like format, which can be
     * 'chr', 'chr:start' and 'chr:start-end'
     * */
    void setRegion(const std::string& region)
    {
        ...
        if (isBcf)
        {
            hidx = std::shared_ptr<hts_idx_t>(
                bcf_index_load(fname.c_str()), details::hts_idx_close());
            if (itr)
                itr.reset();  // reset current region.
            if (region.empty())
                itr = std::shared_ptr<hts_itr_t>(
                    bcf_itr_querys(hidx.get(), header.hdr, "."),
                    details::hts_iter_close());
            else
                itr = std::shared_ptr<hts_itr_t>(
                    bcf_itr_querys(hidx.get(), header.hdr, region.c_str()),
                    details::hts_iter_close());
        }
        else
        {
            tidx = std::shared_ptr<tbx_t>(
                tbx_index_load(fname.c_str()), details::tabix_idx_close());
            if (tidx.get() == NULL)
                throw std::invalid_argument(" no tabix index found!");
            if (itr)
                itr.reset();  // reset
            if (region.empty())
                itr = std::shared_ptr<hts_itr_t>(
                    tbx_itr_querys(tidx.get(), "."), details::hts_iter_close());
            else
                itr = std::shared_ptr<hts_itr_t>(
                    tbx_itr_querys(tidx.get(), region.c_str()),
                    details::hts_iter_close());
        }
        if (itr.get() == NULL)
            throw std::runtime_error(
    ...
    bool getNextVariant(BcfRecord& r)
    {
        ...
        if (itr.get() != NULL)
        {
            if (isBcf)
            {
                ...
                bcf_subset_format(
                    r.header->hdr,
                    r.line.get());  // has to be called explicitly for bcf
            ...
            else
            {
                ...
                {
                    ret = vcf_parse1(
                        &s, r.header->hdr, r.line.get());  // ret > 0, error
                    bcf_unpack(r.line.get(), BCF_UN_ALL);
                }
                return (ret <= 0) && (slen > 0);
            }
...
class BcfWriter
{
   private:
    std::shared_ptr<htsFile> fp;  // hts file
    std::shared_ptr<bcf1_t> b = std::shared_ptr<bcf1_t>(
        bcf_init(),
        details::bcf_line_close());  // variant
    int ret;
   ...
   public:
    ...
    /**
     * @brief          Open VCF/BCF file for writing. The format is infered from
     * file's suffix
     * @param fname    The file name or "-" for stdin/stdout. For indexed files
     * @param version  The output header is constructed with the internal
     * template given a specific version
     */
     ...
     * file's suffix
     * @param fname    The file name or "-" for stdin/stdout. For indexed files
     * @param h        The output header is pointing to the given BcfHeader
     * object
     */
    ...
    void open(const std::string& fname)
    {
        ...
        fp = std::shared_ptr<htsFile>(
            hts_open(fname.c_str(), mode.c_str()), details::hts_file_close());
        if (!fp)
            throw std::invalid_argument("I/O error: input file is invalid");
    }
    ...
    /// initial a VCF header using the internal template given a specific
    /// version. VCF4.1 is the default
    ...
    void initalHeader(const BcfHeader& h) { hp = &h; }

    /// copy header of given VCF and restrict on samples. if samples=="", FORMAT
    /// removed and only sites left
