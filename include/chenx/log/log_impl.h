...
void Logger<eT>::end()
{
    ...
    std::cout << fmt::format("Total time cost: {:.3f} seconds", elapsed.count())
              << std::endl;
}
...
void Logger<eT>::updateColumnWidths(
    ...
    double timeCost)
{
    logLikelihoodWidth = std::max(
        logLikelihoodWidth,
        static_cast<int>(fmt::format("{:.4f}", logLikelihood).length()));

    // Adjust variance width for each variance value
    for (const eT& var : variances)
    {
        varianceWidth = std::max(
            varianceWidth,
            static_cast<int>(fmt::format("{:.4f}", var).length()));
    }
    for (std::string_view var_name : _var_names)
    {
        varianceWidth
            = std::max(varianceWidth, static_cast<int>(var_name.length()));
    }
    timeCostWidth = std::max(
        timeCostWidth,
        static_cast<int>(fmt::format("{:.3f}", timeCost).length()));
