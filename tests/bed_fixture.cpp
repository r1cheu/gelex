...
uint8_t genotype_to_code(double value)
{
    ...
    throw ArgumentValidationException(
        std::format("Invalid genotype value: {}, must be 0.0, 1.0, 2.0, or NaN", value));
}
...
std::filesystem::path BedFixture::create_random_bed_files(
    ...
    uint64_t seed)
{
    ...
    {
        throw ArgumentValidationException("Missing rate must be in [0.0, 1.0] range");
    }
    if (maf_min < 0.0 || maf_min > 0.5)
    {
        throw ArgumentValidationException("Minimum MAF must be in [0.0, 0.5] range");
    }
    if (maf_max < 0.0 || maf_max > 0.5)
    {
        throw ArgumentValidationException("Maximum MAF must be in [0.0, 0.5] range");
    }
    if (maf_min > maf_max)
    {
        throw ArgumentValidationException("Minimum MAF cannot be greater than maximum MAF");
    }
    ...
    for (Eigen::Index snp_idx = 0; snp_idx < num_snps; ++snp_idx)
    {
        ...
        for (Eigen::Index sample_idx = 0; sample_idx < num_samples; ++sample_idx)
        {
            if (missing_dist(rng_) < missing_rate)
            {
                genotypes(sample_idx, snp_idx) = std::numeric_limits<double>::quiet_NaN();
                continue;
            }
    ...
    return create_bed_files_from_matrix(
        genotypes, snp_ids, sample_ids, chromosomes);
}

std::filesystem::path BedFixture::create_bed_files_from_matrix(
...
{
    validate_genotype_matrix(genotype_matrix);

    const Eigen::Index num_samples = genotype_matrix.rows();
    const Eigen::Index num_snps = genotype_matrix.cols();

    validate_id_span_size(snp_ids, num_snps, "SNP ID");
    validate_id_span_size(sample_ids, num_samples, "Sample ID");
    validate_id_span_size(chromosomes, num_snps, "Chromosome");

    current_prefix_ = file_fixture_.generate_random_file_path();
    auto bed_path = current_prefix_;
    bed_path.replace_extension(".bed");

    write_bed_file(genotype_matrix);
...
std::vector<std::byte> BedFixture::encode_variant(const Eigen::VectorXd& variant)
{
...
void BedFixture::write_bed_file(const Eigen::MatrixXd& genotypes)
{
    ...
    [[maybe_unused]] auto bed_file_path = file_fixture_.create_named_binary_file(
    ...
    std::span<const std::string> chromosomes)
{
    ...
    {
        std::string chrom = chromosomes.empty() ? "1" : std::string(chromosomes[i]);
        std::string snp_id = snp_ids.empty() ? std::format("rs{}", i + 1) : snp_ids[i];
...
void BedFixture::validate_genotype_matrix(const Eigen::MatrixXd& matrix)
{
    if (matrix.rows() == 0 || matrix.cols() == 0)
    {
        throw ArgumentValidationException("Genotype matrix cannot be empty");
    }
    ...
    for (Eigen::Index j = 0; j < matrix.cols(); ++j)
    {
        for (Eigen::Index i = 0; i < matrix.rows(); ++i)
        {
            ...
            if (!std::isnan(val) && val != 0.0 && val != 1.0 && val != 2.0)
            {
                throw ArgumentValidationException(
                    std::format("Genotype value({},{})={} is invalid, must be 0.0, 1.0, 2.0, or NaN",
                                i, j, val));
            }
        }
    }
...
void BedFixture::validate_id_span_size(
    std::span<const std::string> ids,
    Eigen::Index expected_size,
    const std::string& id_type)
{
    if (!ids.empty() && static_cast<Eigen::Index>(ids.size()) != expected_size)
    {
        throw ArgumentValidationException(
            std::format("{} count({}) does not match expected count({})",
                        id_type, ids.size(), expected_size));
    }
}
