    ...
    "[data_pipe][create]")
{
    ...
    SECTION("Partial configuration - qcovariates only")
    {
        ...
        REQUIRE(result->fixed_effects().rows() == 5);
        REQUIRE(
            result->fixed_effects().cols()
            == 1 + 3);  // Intercept + 3 qcovariates
    }
    ...
    "[data_pipe][intersection]")
{
    SECTION("Complete intersection with all samples")
    {
        ...
        REQUIRE(result->fixed_effect_names()[0] == "Intercept");
    ...
    "[data_pipe][move]")
{
    ...
    SECTION("take_fixed_effects method")
    {
        ...
        REQUIRE(fixed_effects.cols() == 1 + 3);  // Intercept + 3 qcovariates
