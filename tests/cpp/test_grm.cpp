...
TEST_CASE("Genetic Relationship Matrix Test", "[Grm]") {
  ...
  SECTION("Initialization") {
    REQUIRE_NOTHROW(gelex::AddGrm(train_bed, BIG_CHUNK_SIZE));
  }

  SECTION("Small chunk size") {
    gelex::AddGrm grm_maker{train_bed, SMALL_CHUNK_SIZE};
    dmat grm{grm_maker.Compute()};
    REQUIRE(!grm.is_zero());
  }
  SECTION("Big chunk size") {
    gelex::AddGrm grm_maker{train_bed, BIG_CHUNK_SIZE};
    dmat grm{grm_maker.Compute()};
    REQUIRE(!grm.is_zero());
  }

  SECTION("Result matches under different chunk size") {
    gelex::AddGrm small_maker{train_bed, SMALL_CHUNK_SIZE};
    dmat small_grm{small_maker.Compute()};

    gelex::AddGrm big_maker{train_bed, BIG_CHUNK_SIZE};
  ...
  SECTION("Check AddGrm Result") {
    gelex::AddGrm grm_maker{train_bed, BIG_CHUNK_SIZE};
    dmat grm{grm_maker.Compute()};
    dmat expected_grm{{0.33333337, -0.33333331, 1.1589792e-08},
  ...
  SECTION("Check AddGrm Center") {
    ...
    REQUIRE(approx_equal(grm_maker.center(), expected, "absdiff", ABS_DIFF));
  }

  SECTION("Check AddGrm Scale factor") {
    gelex::AddGrm grm_maker{train_bed, BIG_CHUNK_SIZE};
    dmat grm{grm_maker.Compute()};
    REQUIRE_THAT(grm_maker.scale_factor(),
                 Catch::Matchers::WithinAbs(2, ABS_DIFF));
  }

  SECTION("Check DomGrm Result") {
    gelex::DomGrm grm_maker{train_bed, 2};
    dmat grm{grm_maker.Compute()};
    dmat expected_grm{{0.88235295, 0.35294119, -0.52941173},
  ...
  SECTION("Check DomGrm Center") {
    gelex::DomGrm grm_maker{train_bed, BIG_CHUNK_SIZE};
    dmat grm{grm_maker.Compute()};
