...
TEST_CASE("CrossGrm computation", "[cross_grm]") {
  ...
  const std::string test_bed = std::string(GELEX_TESTS_DIR) + "/data/test.bed";
  ...
  SECTION("AddCrossGrm computation") {
    ...
    dmat expected_grm = {{3.3333334e-01, -3.3333334e-01, -3.3113690e-09},
  ...
  SECTION("AddCrossGrm chunk computation") {
    ...
    AddCrossGrm add_grm(train_bed, std::move(add_center), add_scale_factor, 2);
    dmat grm = add_grm.Compute(test_bed);
  ...
  SECTION("DomCrossGrm computation") {
    ...
    dmat expected_grm = {{0.882353, 0.35294122, -0.5294118},
  ...
  SECTION("DomCrossGrm Chunk computation") {
    DomCrossGrm dom_grm(train_bed, std::move(dom_center), dom_scale_factor, 2);
    dmat grm = dom_grm.Compute(test_bed);
  ...
  SECTION("CrossGrm with SNP mismatch throws exception") {
    AddCrossGrm add_grm(train_bed, std::move(add_center), add_scale_factor);
    REQUIRE_THROWS_WITH(add_grm.Compute(test_missmatch_bed),
  ...
  SECTION("CrossGrm with excluded individuals") {
    ...
    AddCrossGrm add_grm(train_bed, std::move(center), scale_factor,
                        gelex::DEFAULT_CHUNK_SIZE, exclude_individuals);
