...
TEST_CASE("FamReader - Valid .fam file loading", "[data][reader]")
{
    FileFixture files;
...
4 sample4 3 4 2 2.1
        ...
        REQUIRE_NOTHROW(
            [&]()
            {
                FamReader reader(file_path);

                const auto& ids = reader.ids();
...
TEST_CASE("FamReader - Error conditions", "[data][reader][error]")
{
    FileFixture files;
...
TEST_CASE("FamReader - Method functionality", "[data][reader][methods]")
{
    FileFixture files;
...
3 sample3 1 2 1 3.2
        ...
        // Before move
        const auto& original_ids = reader.ids();
        REQUIRE(original_ids.size() == 3);

        // Move the IDs
        auto moved_ids = std::move(reader).take_ids();
        ...
        // After move, the reader should still be usable but ids() may be empty
        // This depends on implementation, but at minimum shouldn't crash
        REQUIRE_NOTHROW(reader.ids());
        REQUIRE_NOTHROW(reader.data());
    }
}

TEST_CASE("FamReader - Edge cases", "[data][reader][edge]")
{
    FileFixture files;
    SECTION("Edge case - single line")
    {
        ...
        auto file_path = files.create_text_file(fam_content, ".fam");
        REQUIRE_NOTHROW(FamReader(file_path));
    }
    ...
    SECTION("Edge case - .fam file with tab delimiter (should succeed)")
    {
        ...
        REQUIRE_NOTHROW(
            ...
            {
                FamReader reader(file_path);
                REQUIRE(reader.ids().size() == 1);
                REQUIRE(reader.ids()[0] == sid("1", "sample1"));
            }());
    }
}
