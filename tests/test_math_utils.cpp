...
TEST_CASE("centralize works correctly", "[utils]")
{
    ...
    RowVectorXd actual_means = gelex::detail::centralize(x);
...
TEST_CASE("standardize works correctly", "[utils]")
{
    ...
    auto [actual_means, actual_stddevs] = gelex::detail::standardize(x);
...
TEST_CASE("standardize handles constant columns", "[utils]")
{
    ...
    auto [means, stddevs] = gelex::detail::standardize(x);
