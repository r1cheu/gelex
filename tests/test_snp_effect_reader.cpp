...
TEST_CASE(
    "SnpEffectReader - Constructor and basic loading",
    "[data][snp_effect]")
{
    FileFixture files;
    ...
    SECTION("Happy path - load complete file with all columns")
    {
        ...
        REQUIRE_NOTHROW(
            [&]()
            {
                SnpEffectReader reader(file_path);
                const auto& effects = reader.effects();

                REQUIRE(effects.size() == 3);
                REQUIRE(reader.has_dom_effects() == true);
    ...
    SECTION("Happy path - load file without Dom column")
    {
        ...
        REQUIRE_NOTHROW(
            [&]()
            {
                ...
                REQUIRE(effects.size() == 3);
                REQUIRE(reader.has_dom_effects() == false);
    ...
    SECTION("Happy path - take_effects moves data")
    {
        ...
        SnpEffectReader reader(file_path);
        SnpEffects effects = std::move(reader).take_effects();

        REQUIRE(effects.size() == 2);
        REQUIRE(reader.effects().size() == 0);  // Effects should be moved out
    }
}

TEST_CASE("SnpEffectReader - Error handling", "[data][snp_effect]")
{
    FileFixture files;
    ...
    SECTION("Exception path - file with only header")
    {
        ...
        REQUIRE_NOTHROW(
            ...
            {
                SnpEffectReader reader(file_path);
                REQUIRE(reader.effects().size() == 0);
            }());
    }
}

TEST_CASE("SnpEffectReader - Column order variations", "[data][snp_effect]")
{
    FileFixture files;
...
TEST_CASE("SnpEffectReader - Edge cases", "[data][snp_effect]")
{
    FileFixture files;
