...
TEST_CASE(
    ...
    "[data][rank_inverse_norm_transform]")
{
...
TEST_CASE(
    "RankInverseNormTransform - DINT handles ties correctly",
    "[data][rank_inverse_norm_transform]")
{
...
TEST_CASE(
    "RankInverseNormTransform - IINT with covariates",
    "[data][rank_inverse_norm_transform]")
{
    std::mt19937 gen(42);
...
TEST_CASE(
    "RankInverseNormTransform - Custom offset parameter",
    "[data][rank_inverse_norm_transform]")
{
...
TEST_CASE(
    "RankInverseNormTransform - Small sample size",
    "[data][rank_inverse_norm_transform]")
{
...
TEST_CASE(
    "RankInverseNormTransform - Extreme values",
    "[data][rank_inverse_norm_transform]")
{
    Eigen::VectorXd phenotype(100);
    for (int i = 0; i < 100; ++i)
