...
TEST_CASE("PhenotypeSimulator - effect class validation", "[simulate]")
{
    BedFixture fixture;
    auto [bed_path, _] = fixture.create_bed_files(10, 20, 0.0, 0.05, 0.5, 42);

    SECTION("Empty effect classes")
    {
        REQUIRE_THROWS_AS(
            PhenotypeSimulator(make_config(bed_path, 0.5, 0.0, 42, {})),
            ArgumentValidationException);
    }

    SECTION("Proportions do not sum to 1")
    {
        std::vector<gelex::EffectSizeClass> bad = {{0.3, 1.0}, {0.3, 1.0}};
        REQUIRE_THROWS_AS(
            PhenotypeSimulator(make_config(bed_path, 0.5, 0.0, 42, bad)),
            ArgumentValidationException);
    }

    SECTION("Zero variance is allowed")
    {
        std::vector<gelex::EffectSizeClass> ok = {{1.0, 0.0}};
        REQUIRE_NOTHROW(
            PhenotypeSimulator(make_config(bed_path, 0.5, 0.0, 42, ok)));
    }

    SECTION("Negative variance")
    {
        std::vector<gelex::EffectSizeClass> bad = {{1.0, -0.1}};
        REQUIRE_THROWS_AS(
            PhenotypeSimulator(make_config(bed_path, 0.5, 0.0, 42, bad)),
            ArgumentValidationException);
    }

    SECTION("Negative proportion")
    {
        std::vector<gelex::EffectSizeClass> bad = {{-0.5, 1.0}, {1.5, 1.0}};
        REQUIRE_THROWS_AS(
            PhenotypeSimulator(make_config(bed_path, 0.5, 0.0, 42, bad)),
            ArgumentValidationException);
    }

    SECTION("Dominance classes validated when d2 > 0")
    {
        std::vector<gelex::EffectSizeClass> good = {{1.0, 1.0}};
        std::vector<gelex::EffectSizeClass> bad = {{0.3, 1.0}, {0.3, 1.0}};
        REQUIRE_THROWS_AS(
            PhenotypeSimulator(make_config(bed_path, 0.5, 0.2, 42, good, bad)),
            ArgumentValidationException);
    }

    SECTION("Dominance classes not validated when d2 = 0")
    {
        std::vector<gelex::EffectSizeClass> good = {{1.0, 1.0}};
        std::vector<gelex::EffectSizeClass> bad = {{0.3, 1.0}, {0.3, 1.0}};
        REQUIRE_NOTHROW(
            PhenotypeSimulator(make_config(bed_path, 0.5, 0.0, 42, good, bad)));
    }
}
