...
TEST_CASE("split gelman rubin", "[diagnostics]")
{
    ...
    samples[1]
        = Eigen::RowVectorXd::NullaryExpr(10, [&]() { return dist(rng); });
    gelex::stats::Chains repeated_samples(4);
    ...
    auto r_hat1 = gelex::stats::gelman_rubin(repeated_samples);
    auto r_hat2 = gelex::stats::split_gelman_rubin(samples);
...
{
    gelex::stats::Chains x(1);
    x[0] = Eigen::RowVectorXd::LinSpaced(10, 0, 9);

    gelex::stats::Chains random_x(1);
    std::mt19937_64 rng(42);
    std::normal_distribution<double> dist(0, 1);
    random_x[0]
    ...
    SECTION("autocorrelation with unbiased estimator")
    {
        ...
        auto result = gelex::stats::autocorrelation(x, false);
    ...
    SECTION("autocorrelation with biased estimator")
    {
        ...
        auto result = gelex::stats::autocorrelation(x, true);
        REQUIRE(result[0].isApprox(expected, 0.01));
    }

    SECTION("autocorrelation with random data and unbiased estimator")
    {
        auto result = gelex::stats::autocorrelation(random_x, false);
    ...
    {
        auto result = gelex::stats::autocorrelation(random_x, true);
        Eigen::RowVectorXd ac = result[0].rightCols(100);
...
{
    gelex::stats::Chains x;
    x.emplace_back(Eigen::RowVectorXd::LinSpaced(10, 0, 9));
    auto actual = gelex::stats::autocovariance(x, false);
...
TEST_CASE("effective sample size", "[diagnostics]")
{
    ...
    auto result = gelex::stats::effect_sample_size(x, false);
...
TEST_CASE("hpdi", "[diagnostics]")
{
    ...
    auto [left, right] = gelex::stats::hpdi(x, 0.2);
