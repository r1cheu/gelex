...
TEST_CASE("CovarEffectReader Constructor Tests", "[predict][covar_effect]")
{
    FileFixture files;
        ...
        "categorical vars")
    {
        ...
        REQUIRE_NOTHROW(
            [&]()
            {
                CovarEffectReader reader(file_path);
                const auto& covar_effect = reader.effects();
    ...
    SECTION("Happy path - file with only categorical variables")
    {
        ...
        REQUIRE_NOTHROW(
            ...
            {
                CovarEffectReader reader(file_path);
                REQUIRE(reader.effects().intercept == 3.0);
                REQUIRE(reader.effects().continuous_coeffs.empty());

                const auto& categorical = reader.effects().categorical_coeffs;
                REQUIRE(categorical.size() == 2);
    ...
    SECTION("Exception - missing intercept term")
    {
        ...
        REQUIRE_THROWS_AS(
            CovarEffectReader(file_path), gelex::FileFormatException);
    }
...
TEST_CASE("CovarEffectReader Accessor Tests", "[predict][covar_effect]")
{
    FileFixture files;
    ...
    SECTION("intercept() returns correct value")
    {
        ...
        CovarEffectReader reader(file_path);
        REQUIRE(reader.effects().intercept == 3.14159);
    }
    ...
    SECTION("continuous_coeffs() returns correct mapping")
    {
        ...
        CovarEffectReader reader(file_path);
        const auto& coeffs = reader.effects().continuous_coeffs;
    ...
    SECTION("categorical_coeffs() returns correct nested mapping")
    {
        ...
        CovarEffectReader reader(file_path);
        const auto& coeffs = reader.effects().categorical_coeffs;
...
TEST_CASE("CovarEffectReader Error Handling Tests", "[predict][covar_effect]")
{
    FileFixture files;
    ...
    SECTION("Exception - file does not exist")
    {
        ...
        REQUIRE_THROWS(CovarEffectReader(non_existent_path));
    }
