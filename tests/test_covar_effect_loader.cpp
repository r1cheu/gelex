...
TEST_CASE("CovarEffectLoader Constructor Tests", "[predictor][covar_effect]")
{
        ...
        "categorical vars")
    {
        ...
        REQUIRE_NOTHROW(
            [&]()
            {
                ...
                REQUIRE(loader.intercept() == 2.5);

                const auto& continuous = loader.continuous_coeffs();
                REQUIRE(continuous.size() == 2);
                REQUIRE(continuous.at("Age") == 0.5);
                REQUIRE(continuous.at("Height") == -0.2);

                const auto& categorical = loader.categorical_coeffs();
    ...
    SECTION("Happy path - file with only intercept")
    {
        ...
        REQUIRE_NOTHROW(
            ...
            {
                CovarEffectLoader loader(file_path);
                REQUIRE(loader.intercept() == 1.8);
                REQUIRE(loader.continuous_coeffs().empty());
                REQUIRE(loader.categorical_coeffs().empty());
            }());
    }
...
TEST_CASE("CovarEffectLoader Accessor Tests", "[predictor][covar_effect]")
{
    ...
    SECTION("intercept() returns correct value")
    {
        ...
        CovarEffectLoader loader(file_path);
        REQUIRE(loader.intercept() == 3.14159);
    }
    ...
    SECTION("continuous_coeffs() returns correct mapping")
    {
        ...
        CovarEffectLoader loader(file_path);
        const auto& coeffs = loader.continuous_coeffs();
    ...
    SECTION("categorical_coeffs() returns correct nested mapping")
    {
        ...
        CovarEffectLoader loader(file_path);
        const auto& coeffs = loader.categorical_coeffs();
...
TEST_CASE("CovarEffectLoader predict() Tests", "[predictor][covar_effect]")
{
    FileFixture files;

    SECTION("Happy path - predict with complete data")
    {
        auto file_path = files.create_text_file(
            "term\tmean\tstddev\tpercentile_5\tpercentile_95\tess\trhat\n"
            "Intercept\t2.0\t0.1\t1.8\t2.2\t1000\t1.0\n"
            "Age\t0.5\t0.05\t0.4\t0.6\t800\t1.01\n"
            "BMI\t-0.3\t0.03\t-0.36\t-0.24\t900\t1.02\n"
            "Sex_M\t0.2\t0.02\t0.16\t0.24\t700\t1.03\n"
            "Sex_F\t-0.1\t0.02\t-0.14\t-0.06\t750\t1.04\n"
            "Group_A\t0.8\t0.1\t0.6\t1.0\t850\t1.05\n"
            "Group_B\t0.3\t0.08\t0.14\t0.46\t800\t1.06\n");

        CovarEffectLoader loader(file_path);

        IndividualData data;
        data.continuous_values["Age"] = 30.0;
        data.continuous_values["BMI"] = 25.5;
        data.categorical_values["Sex"] = "M";
        data.categorical_values["Group"] = "A";

        double result = loader.predict(data);

        // Expected: intercept + Age*0.5 + BMI*(-0.3) + Sex_M(0.2) +
        // Group_A(0.8) = 2.0 + 30*0.5 + 25.5*(-0.3) + 0.2 + 0.8 = 2.0 + 15.0
        // - 7.65 + 0.2 + 0.8 = 10.35
        double expected = 2.0 + (30.0 * 0.5) + (25.5 * -0.3) + 0.2 + 0.8;
        REQUIRE_THAT(result, WithinAbs(expected, 1e-10));
    }

    SECTION("Happy path - predict with missing continuous variable (allowed)")
    {
        auto file_path = files.create_text_file(
            "term\tmean\tstddev\tpercentile_5\tpercentile_95\tess\trhat\n"
            "Intercept\t1.0\t0.1\t0.8\t1.2\t1000\t1.0\n"
        ...
        IndividualData data;
        data.continuous_values["Age"] = 25.0;  // Height is missing
        // No categorical values

        double result = loader.predict(data);

        // Expected: intercept + Age*0.5 (Height not in data, so not checked)
        // = 1.0 + 25*0.5 = 1.0 + 12.5 = 13.5
        double expected = 1.0 + (25.0 * 0.5);
        REQUIRE_THAT(result, WithinAbs(expected, 1e-10));
    }

    SECTION("Exception - predict with unknown categorical category")
    {
        auto file_path = files.create_text_file(
            "term\tmean\tstddev\tpercentile_5\tpercentile_95\tess\trhat\n"
            "Intercept\t2.0\t0.1\t1.8\t2.2\t1000\t1.0\n"
            "Group_A\t0.5\t0.1\t0.3\t0.7\t800\t1.01\n"
            "Group_B\t0.3\t0.08\t0.14\t0.46\t700\t1.02\n");

        CovarEffectLoader loader(file_path);

        IndividualData data;
        data.categorical_values["Group"] = "C";  // Unknown category

        REQUIRE_THROWS_AS(loader.predict(data), gelex::InvalidInputException);
    }

    SECTION(
        "Exception - predict with unknown categorical variable and category")
    {
        auto file_path = files.create_text_file(
            "term\tmean\tstddev\tpercentile_5\tpercentile_95\tess\trhat\n"
            "Intercept\t1.5\t0.1\t1.3\t1.7\t1000\t1.0\n"
            "Sex_M\t0.2\t0.02\t0.16\t0.24\t800\t1.01\n");

        CovarEffectLoader loader(file_path);

        IndividualData data;
        data.categorical_values["UnknownVar"] = "Value";  // Unknown variable

        REQUIRE_THROWS_AS(loader.predict(data), gelex::InvalidInputException);
    }

    SECTION("Happy path - predict with only intercept")
    {
        auto file_path = files.create_text_file(
            "term\tmean\tstddev\tpercentile_5\tpercentile_95\tess\trhat\n"
            "Intercept\t-1.2\t0.2\t-1.6\t-0.8\t1500\t1.0\n");
    ...
    SECTION("Happy path - predict with continuous variable only")
    {
        auto file_path = files.create_text_file(
            "term\tmean\tstddev\tpercentile_5\tpercentile_95\tess\trhat\n"
            "Intercept\t0.0\t0.1\t-0.2\t0.2\t1000\t1.0\n"
            "X\t2.0\t0.3\t1.4\t2.6\t800\t1.01\n"
            "Y\t-1.5\t0.2\t-1.9\t-1.1\t900\t1.02\n");

        CovarEffectLoader loader(file_path);

        IndividualData data;
        data.continuous_values["X"] = 3.0;
        data.continuous_values["Y"] = 2.0;

        double result = loader.predict(data);
        // Expected: 0.0 + 3*2.0 + 2*(-1.5) = 0 + 6 - 3 = 3.0

        REQUIRE_THAT(result, WithinAbs(3.0, 1e-10));
    }

    SECTION("Happy path - predict with categorical variable only")
    {
        auto file_path = files.create_text_file(
            "term\tmean\tstddev\tpercentile_5\tpercentile_95\tess\trhat\n"
            "Intercept\t1.0\t0.1\t0.8\t1.2\t1000\t1.0\n"
            "Color_Red\t0.5\t0.1\t0.3\t0.7\t800\t1.01\n"
            "Color_Blue\t-0.2\t0.08\t-0.34\t-0.06\t700\t1.02\n"
            "Color_Green\t0.3\t0.12\t0.06\t0.54\t900\t1.03\n");

        CovarEffectLoader loader(file_path);

        IndividualData data;
        data.categorical_values["Color"] = "Blue";

        double result = loader.predict(data);
        // Expected: 1.0 + (-0.2) = 0.8
        REQUIRE_THAT(result, WithinAbs(0.8, 1e-10));
    }

    SECTION("Happy path - predict with multiple categorical variables")
    {
        auto file_path = files.create_text_file(
            "term\tmean\tstddev\tpercentile_5\tpercentile_95\tess\trhat\n"
            "Intercept\t0.0\t0.1\t-0.2\t0.2\t1000\t1.0\n"
            "Var1_A\t1.0\t0.2\t0.6\t1.4\t800\t1.01\n"
            "Var1_B\t0.5\t0.15\t0.2\t0.8\t700\t1.02\n"
            "Var2_X\t2.0\t0.3\t1.4\t2.6\t900\t1.03\n"
            "Var2_Y\t-1.0\t0.25\t-1.5\t-0.5\t850\t1.04\n");

        CovarEffectLoader loader(file_path);

        IndividualData data;
        data.categorical_values["Var1"] = "B";
        data.categorical_values["Var2"] = "X";

        double result = loader.predict(data);
        REQUIRE_THAT(result, WithinAbs(2.5, 1e-10));
    }
}
