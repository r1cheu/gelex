...
class Environment:  # noqa: PLW1641
    ...
    def capture(cls, env=0, reference=0):
        if isinstance(env, cls):
            return env
        if isinstance(env, numbers.Integral):
            depth = env + reference
        else:
            msg = (
                "'env' must be either an integer or an instance of Environment."
            )
            raise TypeError(msg)
        ...
        try:
            for _ in range(depth + 1):
                if frame is None:
                    msg = "call-stack is not that deep!"
