...
REMLLoop<eT>::REMLLoop(
    ...
    _blup = Mat<eT>(_Z[0].n_cols, static_cast<uword>(_Z.size()), fill::value(datum::nan));
}
...
void REMLLoop<eT>::_init_var_updater(
    ...
    if (method == "AI") {
        ...
        throw std::invalid_argument("Invalid optim method, please choose from `AI`, `NR`, `Fisher`");
    }
}
...
template <typename eT>
eT REMLLoop<eT>::_cal_loglik(const double& logdet_v, const Mat<eT>& txvx, const Mat<eT>& proj_y) {
...
void REMLLoop<eT>::run(
    ...
    eT logL = _cal_loglik(mat_updater.getLogdet_v(), mat_updater.getTxvx(), mat_updater.getProj_y());
    eT log_diff = 1e100;
    size_t iteration = 1;
    ...
    if (em_init) {
        ...
        varcomp = em_updater.update(mat_updater.getProj_y(), mat_updater.getPdv());
        mat_updater.update(varcomp);
        logL = _cal_loglik(mat_updater.getLogdet_v(), mat_updater.getTxvx(), mat_updater.getProj_y());
    ...
    for (; iteration < max_iteration; ++iteration) {
        ...
        varcomp = var_updater->update(mat_updater.getProj_y(), mat_updater.getPdv());
        mat_updater.update(varcomp);

        eT new_logL = _cal_loglik(mat_updater.getLogdet_v(), mat_updater.getTxvx(), mat_updater.getProj_y());
    ...
    std::cout << (_converged ? "Converged!!!" : "Not converged!!! Try to increase num of iteration") << std::endl;
}
