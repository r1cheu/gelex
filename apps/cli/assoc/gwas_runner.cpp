...
{
    if (!gelex::is_center_family_method(method))
    {
    ...
    if (is_additive)
        gelex::process_matrix<gelex::GeneticEffectType::Add>(
            method, genotype, freqs);
    else
        gelex::process_matrix<gelex::GeneticEffectType::Dom>(
            method, genotype, freqs);
}
