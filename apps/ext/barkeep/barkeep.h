...
class AsyncDisplayer
{
   protected:
    ...
    BaseDisplay* parent_
        = nullptr;  ///< parent display to render which owns this
    ...
    /// @param redraw If true, display is assumed to be redrawn. This, e.g.
    /// means
   ...
   public:
    ...
    void show()
    {
        ...
        {
            return;
        }  // noop if already show()n before
        assert(interval_ > Duration{0.});

        displayer_thread_ = std::make_unique<std::thread>(
            [this]()
            {
                display_();
                while (true)
                {
                    ...
                    auto start = std::chrono::steady_clock::now();
                    {
                        ...
                        while (not done and interval >= Duration{0.})
                        {
                            ...
                            interval -= std::chrono::duration_cast<Duration>(
                                elapsed);
                    ...
                    if (done)
                    {
                        // Final newline to avoid overwriting the display with
                        // subsequent output
                        *out_ << std::endl;
                        break;
                    }
...
class BaseDisplay
{
   protected:
    ...
    /// means
    ///               an Animation should not increment the still frame index.
    /// @param end    String to append at the end of the display. We typically
    /// add
...
class AnimationDisplay : public BaseDisplay
{
   ...
   public:
        ...
        : BaseDisplay(cfg.out, Duration{0.}, cfg.message, "", cfg.no_tty)
    {
        ...
        {
            auto idx = static_cast<unsigned short>(
        ...
        displayer_->interval(
            as_duration(cfg.interval) == Duration{0}
...
template <typename T>
using signed_t = typename std::conditional_t<
    std::is_integral_v<T>,
    std::make_signed<T>,
...
class Speedometer
{
   private:
    ...
    double progress_increment_sum_
        = 0;  // (weighted) sum of progress increments
    Duration
        duration_increment_sum_{};  // (weighted) sum of duration increments
   ...
   public:
    double speed()
    {
        ...
        duration_increment_sum_
            = (1 - discount_) * duration_increment_sum_ + dur;
    ...
    {
        std::stringstream ss;  // use local stream to avoid disturbing `out`
                               // with std::fixed and std::setprecision
...
class CounterDisplay : public BaseDisplay
{
   ...
   public:
    ...
    CounterDisplay(
        provider_t<Progress> progress_provider,
    ...
    CounterDisplay(Progress progress_provider, const CounterConfig& cfg = {})
        : CounterDisplay(
              provider_t<Progress>(std::move(progress_provider)),
              cfg)
    {
    }
...
class ProgressBarDisplay : public BaseDisplay
{
   ...
   protected:
    ...
    void render_progress_bar_(std::ostream* out)
    {
        ...
        size_t partial = size_t(
            ValueType(bar_parts_.fill.size()) * ValueType(width_)
        ...
        *out << bar_parts_.left;
        *out
            << (complete ? bar_parts_.complete_left_modifier
                         : bar_parts_.incomplete_left_modifier);

        // filled portion
        for (int i = 0; i < on; i++)
        {
            *out << bar_parts_.fill.back();
        }

        // partially filled character
        if (partial > 0)
        {
            *out << bar_parts_.fill.at(partial - 1);
        }
    ...
    /// Progress width is expanded (and right justified) to match width of
    /// total.
    void render_counts_(const std::string& end = " ")
    {
    ...
    void render_percentage_(const std::string& end = " ")
    {
        ...
        ss << std::fixed << std::setprecision(2);
        ss.width(6);
        ss << std::right << progress_provider_.load() * 100. / total_ << "%"
           << end;
        out() << ss.str();
    }
