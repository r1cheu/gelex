...
.. _gwas-joint-cov-format:

GWAS Joint Covariance Sidecar (.cov)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Generated by ``assoc --write-cov`` for
``--test joint --geno-method C``. Contains the per-SNP variance and covariance
terms for the additive and dominance effect estimates.
...
.. list-table::
   ...
   * - ``A1FREQ``
     - Effect allele frequency.
   * - ``BETA_A, BETA_D``
     - Additive and dominance effect estimates from the joint test.
   * - ``VAR_A, VAR_D, COV_AD``
     - Variance of ``BETA_A``, variance of ``BETA_D``, and their covariance.
